...
import { fetchPropertyAvailability } from '../services/bookingService'
...
function buildDayStatusMap(days, occupiedDates) {
  const map = new Map()
  if (Array.isArray(days)) {
    for (const row of days) {
      if (row?.date && row?.status) {
        map.set(String(row.date), String(row.status).toLowerCase())
      }
    }
  }
  if (Array.isArray(occupiedDates)) {
    for (const ymd of occupiedDates) {
      if (ymd && !map.has(ymd)) {
        map.set(ymd, 'occupied')
      }
    }
  }
  return map
...
function getDayAvailability(date, inMonth, listingMode, today, dayStatusMap) {
  if (!inMonth) return 'other'

  const ymd = dateToYMD(stripTime(date))
  const fromApi = dayStatusMap.get(ymd)
...
const DAY_CELL_CLASS = {
  ...
  past: 'border-[#4A5568]/20 bg-[#EDF2F7] text-[#718096] cursor-default',
...
const LEGEND = [
  ...
  { key: 'past', label: 'Past', className: 'bg-[#EDF2F7] border-[#4A5568]/20' },
]

/**
 * Centralized property availability calendar.
 * @param {number|string} propertyId
 * @param {number|string} [bookingId] — bumps refresh when booking payment completes
 * @param {'student'|'landlord'} [viewMode]
 */
export default function AvailabilityCalendar({
  ...
  const resolvedPropertyId = propertyId != null && propertyId !== '' ? Number(propertyId) : null
  ...
  const dayStatusMap = useMemo(
    () => buildDayStatusMap(dayRows, occupiedDates),
    [dayRows, occupiedDates],
  )

  useEffect(() => {
    if (!resolvedPropertyId || Number.isNaN(resolvedPropertyId)) {
      setDayRows([])
      setOccupiedDates([])
      setLoadError('')
      return undefined
    ...
    setLoadError('')
    fetchPropertyAvailability(resolvedPropertyId, {
      year: cursor.year,
      month: cursor.month + 1,
    })
      .then((data) => {
        if (cancelled) return
        setDayRows(Array.isArray(data?.days) ? data.days : [])
        setOccupiedDates(Array.isArray(data?.occupiedDates) ? data.occupiedDates : [])
      })
      .catch((err) => {
        if (cancelled) return
        setDayRows([])
  ...
  }, [resolvedPropertyId, cursor.year, cursor.month, refreshKey, bookingId])
  ...
  const cells = useMemo(() => {
    ...
    for (let i = 0; i < 42; i += 1) {
      ...
      const dayKey = getDayAvailability(date, inMonth, listingMode, today, dayStatusMap)
      const isToday = stripTime(date).getTime() === today.getTime()
      const ymd = dateToYMD(stripTime(date))
      const inLeaseRange =
        moveInYmd && moveOutYmd && ymd >= moveInYmd && ymd <= moveOutYmd
  ...
  const selectedLabel = onDaySelect
    ? [moveInYmd ? `Move-in: ${moveInYmd}` : null, moveOutYmd ? `Move-out: ${moveOutYmd}` : null]
        .filter(Boolean)
  ...
  const footerNote =
    viewMode === 'landlord'
      ? 'Red dates are locked by confirmed tenant bookings.'
      : 'Red dates are already booked. Green dates are open for new applications.'
    ...
    <div className="rounded-xl border border-[#E2E8F0] bg-white p-6 shadow-sm">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <h3 className="text-center text-lg font-bold text-[#2D3748]">
          ...
          {loadingAvailability ? (
            <span className="ml-2 text-xs font-normal text-[#A0AEC0]">Updating…</span>
          ) : null}
        </h3>
        <button
          type="button"
      ...
      <div className="grid grid-cols-7 gap-1.5 sm:gap-2" aria-label="Availability calendar">
        ...
        {cells.map((cell) => {
          const clickable =
            cell.inMonth && cell.dayKey !== 'past' && cell.dayKey !== 'other' && cell.dayKey !== 'occupied'
          const showLeasePick =
            onDaySelect && (cell.isMoveIn || cell.isMoveOut) && cell.dayKey !== 'occupied'
          return (
            <button
              key={`${cell.date.getFullYear()}-${cell.date.getMonth()}-${cell.date.getDate()}`}
              type="button"
              disabled={!clickable && !showLeasePick}
              ...
              className={[
                ...
                cell.inLeaseRange && cell.dayKey === 'available'
                  ? 'bg-[#F3F0FF] border-[#6C2BD9]/30'
                  : '',
                cell.isToday ? 'ring-2 ring-[#F59E0B] ring-offset-1' : '',
                cell.isSelected && (clickable || showLeasePick)
                  ? 'ring-2 ring-[#6C2BD9] ring-offset-1 font-bold'
                  : '',
                showLeasePick ? 'bg-[#6C2BD9] text-white border-[#6C2BD9]' : '',
      ...
      {hideFooterNote ? null : (
        <p className="mt-3 text-xs text-[#A0AEC0]">
          {resolvedPropertyId ? footerNote : 'Open a property to load live availability.'}
        </p>
      )}
    </div>
  )
