...
public class AuthService {
    ...
    @Value("${app.auth.jwt-secret:mySecretKey}")
    ...
    public AuthService(UserAccountRepository userAccountRepository, MailService mailService, IcCryptoService icCryptoService) {
    ...
    public UserAccount register(RegisterRequest request) {
        ...
        if (!SELF_SERVE_ROLES.contains(roleNorm)) {
    ...
    public UserAccount login(LoginRequest request) {
        ...
        String rawLogin = request.email.trim();
        String lookupEmail = rawLogin;
        if ("admin".equalsIgnoreCase(rawLogin) && configuredAdminEmail != null && !configuredAdminEmail.trim().isEmpty()) {
    ...
    /**
     * Updates editable profile fields for the signed-in student or landlord. Email, password, role, and IC are unchanged.
     */
    public UserAccount updateProfile(String authorization, ProfileUpdateRequest request) {
        ...
        if (!SELF_SERVE_ROLES.contains(role)) {
            throw new IllegalArgumentException("Profile update is only available for student and landlord accounts.");
        }
        if (request == null) {
    ...
    public void forgotPassword(ForgotPasswordRequest request) {
        ...
        userAccountRepository.findByEmailIgnoreCase(request.email.trim().toLowerCase())
                .ifPresent(user -> {
    ...
    private void validateRegisterRequest(RegisterRequest request) {
        ...
        if (!PHONE_PATTERN.matcher(normalizedPhone).matches()) {
            throw new IllegalArgumentException("Phone Number must follow format 12-3456789");
        }
